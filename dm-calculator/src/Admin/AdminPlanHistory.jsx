...
const handleSave = () => {
  ...
  const request = editId
    ? axios.put(`${baseURL}/auth/api/calculator/updatePlanData/${editId}`, totalPayload)
    : axios.post(`${baseURL}/auth/api/calculator/saveCalculatorDataOfPlan`, totalPayload);
...
>
  ← Go Back
        ...
        <div>
  <h3 className="text-xl font-bold text-white">📢 Ads Campaign Budget</h3>
  {loading && (
  ...
  <input
    ...
    onChange={(e) =>
      setEnteredAmount((prev) => ({ ...prev, ["Google Ad"]: e.target.value }))
    }
  />
  ...
  <input
    ...
    onChange={(e) =>
      setEnteredAmount((prev) => ({ ...prev, ["Meta Ad"]: e.target.value }))
    }
  />
</div>
...
{selectedService === "Complimentary" && optionalServices?.length > 0 && (
  <div className="space-y-4">
    {optionalServices.map((opt) => {
      const key = opt.editing_type_name.toLowerCase().replace(/\s+/g, "_");
      return (
    ...
    {Number(order.include_thumbnail_creation) > 0 && (
      <>🖼 Thumbnail Creation </>
                    ...
                    {(order.percent_ads) && (
                      <div className="text-base text-white/60 italic">
                         <p>
                      💰 Budget: ₹{parseFloat(order.amount_ads).toLocaleString()}
                    </p>
                    <p>
                      📊 Charge: ₹{parseFloat(order.charge_ads).toLocaleString()} (
                      {order.percent_ads}%)
                    </p>
                      </div>
                    )}
                  ...
                  <div className="flex items-center gap-2 sm:gap-4">
                    <div className="text-green-400 font-bold text-xl">
                      ₹{parseFloat(order.total_amount||order.total_ads).toLocaleString()}
                    </div>
                    <button
